...
from models.modules.ZeroIIformer import IIformer
...
class IIFormerCmFlow(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        if opt['cond_encoder'] == 'ConEncoder1':
            self.RRDB = IIformer(dd_in=3,embed_dim=48)
        ...
        self.flowUpsamplerNet = \
            FlowUpsamplerNet((self.crop_size, self.crop_size, 3),  K,
    ...
    @autocast()
    def forward(self, gt=None, lr=None, z=None, zc=None,eps_std=None, reverse=False, epses=None, reverse_with_grad=False,
                lr_enc=None,add_gt_noise=True, step=None, y_label=None, align_condition_feature=False, get_color_map=False):
        if get_color_map:
        ...
        if not reverse:
            ...
            if reverse_with_grad:
                return self.reverse_flow(lr, z, zc,y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
    ...
    def reverse_flow(self, lr, z, zc,y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True):
