...
class FlowStep(nn.Module):
    ...
    def __init__(self, in_channels, fFeatures_firstConv,
                 actnorm_scale=1.0, flow_permutation="invconv", flow_coupling="additive",
                 LU_decomposed=False, opt=None, image_injector=None,  normOpt=None, in_shape=None,
                 position=None, image_shape=None, H=None, W=None):
        # check configures
...
###重要，这边可以修改输入NN的通道数
        ...
        if flow_coupling == "CondAffineSeparatedAndCond":
            self.affine = models.modules.FlowAffineCouplingsAblation.CondAffineSeparatedAndCond(in_channels=in_channels,opt=opt,
                                                                                    fFeatures_firstConv=fFeatures_firstConv)#, image_shape=None, H=self.H, W=self.W)
...
#####重要
    def normal_flow(self, z, logdet, rrdbResults=None):
        ...
        # 2. permute
        z, logdet = FlowStep.FlowPermutation[self.flow_permutation](   #invconv
            self, z, logdet, False)
        ...
        if need_features or self.flow_coupling in ["condAffine", "condFtAffine", "condNormAffine"]:
            img_ft = getConditional(rrdbResults, self.position)#第一个层级的特征图是fea_up2,288*64*64，320*48*48
