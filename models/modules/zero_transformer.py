...
class Restormer(nn.Module):
    def __init__(self, inp_channels=3, dim=48, num_refinement_blocks=2, heads=[2, 2, 4, 8],
                 ...
                 depths=[2, 2, 4, 6, 4, 2, 2], num_heads=[1, 2, 4, 8, 8, 4, 2],
    ...
    def forward(self, inp_img, zc):
        # torchvision.utils.save_image(inp_img, "F:\Restormer-main\Result\\inp_img.png")
        result = {}
        ...
        # out = self.output(out_dec_level0) + inp_img  # out用于计算损失
        # # out = self.skip(out)
        # result['color_map'] = out

        # for i in range(3):
        #     # latent_output = torch.mean(z, dim=1)
        #     latent_output = out[:, i, :, :]
        #     latent_output = latent_output.detach()
        #     latent_output = rgb(latent_output)
        #     cv2.imwrite(r"D:\LLflow_different_V\LLflow_Restormer\results\LOLv2-pc/Cecoder_{}.png".format(i), latent_output)
