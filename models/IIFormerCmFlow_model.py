...
class IIFormerCmFlowModel(BaseModel):
    def __init__(self, opt, step):
        super(IIFormerCmFlowModel, self).__init__(opt)
        self.opt = opt
        ...
        #self.cri_gan = lossfn.GANLoss().to(self.device)

        self.netF = lossfn.VGGFeatureExtractor(feature_layer=34, use_bn=False,use_input_norm=True, device=self.device)
        #self.netD = lossfn.NoNormDiscriminator().to(self.device)
        if opt['dist']:
        ...
        #         r"E:\TAO\LLflow_differentV\LLflow_Restormer_simple\experiments\UpSkip_channlCat336_blocks46884_GT160_k20_Onlyz_chan3\models\best_psnr_G.pth"),
        #         strict=False)
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            ...
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, reverse=False, epses=epses,step=step,
                                         zc=self.zero_channel,
                                         align_condition_feature=opt_get(self.opt,['align_condition_feature']) or False)
            #tensor(-6.7288, device='cuda:0', grad_fn= < MeanBackward0 >) * 1
            nll_loss = torch.mean(nll)
            nll_loss = nll_loss * weight_fl
            losses['nll_loss'] = nll_loss

        if weight_l1 > 0 and step > 5:
            #self.optimizer_G.zero_grad()
            z = self.get_z(heat=0, seed=None, batch_size=self.var_L.shape[0], lr_shape=self.var_L.shape)
            sr, logdet = self.netG(lr=self.var_L, z=z, zc=self.zero_channel,
                                   eps_std=0,reverse=True,add_gt_noise=True,reverse_with_grad=True)
            sr = sr.clamp(0, 1)
            if not torch.isnan(sr).any() and (not torch.isinf(sr).any()):

                real_fea = self.netF(self.real_H).detach()
                fake_fea = self.netF(sr)
                l_g_fea = self.cri_pix(fake_fea, real_fea)
                vgg_loss = l_g_fea


                if vgg_loss <= 10.0:
                    #losses['loss_L1'] = l1_loss
                    losses['loss_vgg'] = vgg_loss

                else:
                    losses['loss_L1'] = torch.tensor(0)
                    losses['loss_vgg'] = torch.tensor(0)
        ...
        nll = losses['nll_loss'].item()
        # l1 = losses['loss_L1'].item()
        # vggloss = losses['loss_vgg'].item()
        # Gloss=losses['loss_G'].item()
        # Dloss=losses['loss_D'].item()
        return mean, nll#, l1,vggloss
