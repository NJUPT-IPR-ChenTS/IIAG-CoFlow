...
from test import impad2,pad_tensor_back
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step<2:
                ...
                mean, nll = model.optimize_parameters(current_step)
            ...
            if current_step % opt['logger']['print_freq'] == 0 \
                ...
                message = '<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, eta:{:.4f}, mean:{:.4f},nll:{:.4f}'.format(
                    epoch, current_step, model.get_current_learning_rate(),eta(avg_time),
                    mean, nll)
                print(message)
            timer.tick()
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0 and current_step>0:
                ...
                for val_data in val_loader:
                    ...
                    util.mkdir(img_dir)
                    b, c, h, w = val_data['LQ'].shape
                    if h % opt['pad_factor'] != 0 or w % opt['pad_factor'] != 0:
                        val_data['LQ'], pad_left, pad_right, pad_top, pad_bottom = impad2(val_data['LQ'],opt['pad_factor'])
                        val_data['zero_img'], pad_left, pad_right, pad_top, pad_bottom = impad2(val_data['zero_img'],opt['pad_factor'])
                    ...
                    if h % opt['pad_factor'] != 0 or w % opt['pad_factor'] != 0:
                        ...
                        visuals['NORMAL'] = pad_tensor_back(visuals['NORMAL'], pad_left, pad_right, pad_top, pad_bottom)
                        visuals['LQ'] = pad_tensor_back(visuals['LQ'], pad_left, pad_right, pad_top, pad_bottom)
                ...
                # log
                logger.info('# Validation # PSNR: {:<4.5f} SSIM: {:<4.6f}'.format(avg_psnr, avg_ssim))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:<4.4f} SSIM: {:<4.4f}'.format(
